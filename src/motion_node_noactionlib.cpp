 ...
 * Description: This is an implementation of an autonomous navigated robot, that also can avoid obstacles, through QR code tag detection (no actionlib).
 * 
 */
...
{
private:
	ros::Publisher goalPublisher;			// Sends goals in the map
	...
	ros::Subscriber goalSubscriber;			// Receives goal state codes
...
public:
	FactoryRobot()
	{
		...
		goalPublisher = n.advertise<geometry_msgs::PoseStamped>("/move_base_simple/goal", 10);
		...
		goalSubscriber = n.subscribe("/move_base/result", 10, &FactoryRobot::goalCallback, this);
		...
		ROS_INFO("\n******** Factory Robot Node (No actionlib) ***********\n");
	}
	...
	// Feedback after the end of a goal
	...
	// Goal callback runs when a goal reached, aborted or aborted and go to initial position
	void goalCallback(const move_base_msgs::MoveBaseActionResult::ConstPtr &msg)
	{
		int status = msg->status.status;
		if (status == 3)
		{
			// Goal reached and then start to search the QR tag
			goalFeedback(6, 3, 3);
	...
	void setGoal(float x, float y, float z, float w)
	{
		...
		geometry_msgs::PoseStamped newStamp;
		newStamp.header.stamp = now;
		newStamp.header.frame_id = "map";
		newStamp.pose.position.x = x;
		newStamp.pose.position.y = y;
		newStamp.pose.orientation.z = z;
		newStamp.pose.orientation.w = w;
		goalPublisher.publish(newStamp);
	}
...
{
	ros::init(argc, argv, "motion_node_noactionlib");
	FactoryRobot robot;
	ros::spin();
	return 0;
}
